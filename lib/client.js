...
function composerInput() {
	return composerTextarea() ?? document.querySelector("[data-composer-card] [data-composer-input]");
}
function composerReady() {
	const el = composerInput();
	if (el === null) return false;
	if (el instanceof HTMLTextAreaElement) return !el.disabled && !el.readOnly;
	return el.getAttribute?.("aria-disabled") !== "true" && (el.isContentEditable === true || el.getAttribute?.("contenteditable") === "true");
}
...
const intakeSeqBySession = /* @__PURE__ */ new Map();
function nextIntakeSeq(sessionId) {
...
function peekIntakeSeq(sessionId) {
...
function CacheSettings() {
	...
	const refreshCfg = useCallback(async () => {
		...
		if (scope) try {
			...
			if (value && typeof value === "object" && (value.engine !== void 0 || value.ocr !== void 0)) {
				setCfg(value);
	...
	const saveCfg = useCallback(async (patch) => {
		...
		const scope = officialScope();
		if (scope) try {
			if (typeof scope.update === "function") await scope.update(patch, cfgRevisionRef.current);
			else if (typeof scope.mutate === "function") {
				const ops = [];
				for (const [k, v] of Object.entries(patch)) ops.push({
					op: "replace",
					path: k,
					value: v
				});
				await scope.mutate(ops, cfgRevisionRef.current);
			} else if (typeof scope.set === "function") for (const [k, v] of Object.entries(patch)) if (v !== null && typeof v === "object" && !Array.isArray(v)) for (const [sk, sv] of Object.entries(v)) await scope.set(`${k}.${sk}`, sv);
						...
						children: [jsx("input", {
							...
							onChange: (e) => {
								...
								setCfg((c) => ({
									...
									ocr: {
										...c.ocr,
										crossCloudFallback: v
									}
								}));
								saveCfg({ ocr: { crossCloudFallback: v } });
							}
