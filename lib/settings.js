 ...
 * 密钥优先迁入宿主 credentials 服务；旧宿主无该服务时才以 0600 配置文件
 * 回退保存。返回前端时始终脱敏。
 */
...
export function normalize(raw) {
  ...
  if (ocr !== null && typeof ocr === "object") {
    ...
    if (typeof ocr.crossCloudFallback === "boolean") out.ocr.crossCloudFallback = ocr.crossCloudFallback;
...
export async function loadSettingsDoc(cwd, ctx) {
  ...
  const path = configPath(cwd);
  let file = await readConfigFile(path);
  // 旧版曾把配置镜像到工作区；只读兼容一次，下一次保存会归一到主路径。
  if (file === null && typeof cwd === "string" && isAbsolute(cwd)) {
    const alt = join(cwd, ".dsh-attachments", CONFIG_FILE);
    if (alt !== path) file = await readConfigFile(alt);
  }
...
export async function saveSettings(cwd, patch, expectedRevision, ctx) {
  ...
  if (patch.ocr) {
    ...
    if (typeof patch.ocr.crossCloudFallback === "boolean") next.ocr.crossCloudFallback = patch.ocr.crossCloudFallback;
  }
  if (patch.docServer) {
