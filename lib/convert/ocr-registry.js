...
export async function runOcrChain(rendered, policy, ctx, hooks = {}) {
  ...
  const mayTryRemote = () => policy.ocr !== "auto" || policy.crossCloudFallback === true || !remoteAttempted;
  const markRemoteAttempt = () => { remoteAttempted = true; };
  ...
  // Baidu
  if (policy.baidu?.apiKey && policy.baidu?.secretKey && want("baidu") && mayTryRemote()) {
    markRemoteAttempt();
    try {
  ...
  // VLM (generic)
  if (policy.vlm?.configured && want("vlm") && mayTryRemote()) {
    markRemoteAttempt();
    try {
      const jpegPages = await getJpeg();
  ...
  // Aliyun
  if (policy.aliyun?.appCode && want("aliyun") && mayTryRemote()) {
    markRemoteAttempt();
    try {
      const jpegPages = await getJpeg();
  ...
  // Tencent
  if (policy.tencent?.secretId && policy.tencent?.secretKey && want("tencent") && mayTryRemote()) {
    markRemoteAttempt();
    try {
      const jpegPages = await getJpeg();
  ...
  // Azure
  if (policy.azure?.apiKey && policy.azure?.endpoint && want("azure") && mayTryRemote()) {
    markRemoteAttempt();
    try {
      const jpegPages = await getJpeg();
  ...
  // Volc
  if (policy.volc?.appCode && want("volc") && mayTryRemote()) {
    markRemoteAttempt();
    try {
      const jpegPages = await getJpeg();
  ...
  if ((dsExplicit || (policy.ocr === "auto" && dsAutoOk)) && mayTryRemote()) {
    let dsKey = "";
    try {
