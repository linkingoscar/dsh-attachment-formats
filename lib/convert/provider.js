 ...
 */
export async function enginePolicyWithSettings(cwd) {
  ...
  try {
    ...
    // 扩展供应商通过 settings 透传
    base.aliyun = {
      appCode: String(cfg.ocr?.aliyun?.accessKeyId ?? cfg.ocr?.aliyun?.appCode ?? "").trim() || String(cfg.ocr?.aliyun?.accessKeySecret ?? "").trim(),
      raw: cfg.ocr?.aliyun ?? {}
    };
    base.tencent = {
      secretId: String(cfg.ocr?.tencent?.secretId ?? "").trim(),
      secretKey: String(cfg.ocr?.tencent?.secretKey ?? "").trim(),
      region: String(cfg.ocr?.tencent?.region ?? "ap-guangzhou").trim() || "ap-guangzhou"
    };
    base.azure = {
      endpoint: String(cfg.ocr?.azure?.endpoint ?? "").trim(),
      apiKey: String(cfg.ocr?.azure?.apiKey ?? "").trim()
    };
    base.volc = {
      appCode: String(cfg.ocr?.volc?.accessKey ?? cfg.ocr?.volc?.appCode ?? "").trim()
    };
    base.deepseek = {
      key: String(cfg.ocr?.deepseek?.key ?? "").trim(),
