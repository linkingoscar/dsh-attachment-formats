...
export function enginePolicy() {
  ...
  return {
    ...
    crossCloudFallback: process.env.DSH_ATTACH_CROSS_CLOUD_FALLBACK === "1",
...
export async function enginePolicyWithSettings(cwd, ctxLike) {
  ...
  try {
    ...
    const cfg = await loadSettings(cwd, ctxLike);
    // engine
    ...
    base.crossCloudFallback = cfg.ocr?.crossCloudFallback === true || base.crossCloudFallback === true;
    // vlm 来自设置页覆盖 env
