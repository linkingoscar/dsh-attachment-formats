...
const MAX_BODY_BYTES = 160 * 1024 * 1024; // legacy base64 JSON compatibility only
const MAX_BINARY_BODY_BYTES = MAX_FILE_BYTES;
...
async function convertFile(ctx, file, cwd, directLimit, progress = null) {
  ...
  if (file === null || typeof file !== "object"
    || (!(Buffer.isBuffer(file.bytes)) && (typeof file.data !== "string" || file.data === ""))) {
    return { input, ...fail("empty-file", "文件内容为空") };
  }
  let bytes;
  try {
    bytes = Buffer.isBuffer(file.bytes) ? file.bytes : b64decode(file.data);
  } catch {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (contentType.startsWith("application/octet-stream")) {
            ...
            const bytes = await readBody(req, MAX_BINARY_BODY_BYTES);
            payload = {
              cwd: url.searchParams.get("cwd") ?? undefined,
              sessionId: url.searchParams.get("sessionId") ?? undefined,
              directLimitChars: Number(url.searchParams.get("directLimitChars")),
            ...
            files = Array.isArray(payload?.files) ? payload.files.slice(0, 24) : [];
          }
          if (files.length === 0) {
      ...
      handler: async (req, res) => {
        ...
        if (req.method === "POST") {
          try {
            ...
            // 先把密钥移入 credential store，再写普通设置，避免崩溃窗口把密钥短暂落盘。
            // 服务不可用/拒绝时才保留原字段，兼容旧宿主。
              ...
              for (const [path, ref] of SECRET_FIELDS) {
                ...
                if (typeof value === "string" && value.trim() !== "") {
                  try {
                    ...
                    patch[path[0]][path[1]] = patch[path[0]][path[1]] ?? {};
                    patch[path[0]][path[1]][path[2]] = "";
                    moved += 1;
                  } catch {
            ...
            const next = await saveSettings(cwd, patch, expectedRevision, ctx);
