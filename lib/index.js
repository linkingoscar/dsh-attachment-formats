...
import { deepseekOcrPages, resolveDeepSeekKey } from "./convert/ocr-deepseek.js";
import { azureOcrPages } from "./convert/ocr-azure.js";
import { aliyunOcrPages } from "./convert/ocr-aliyun.js";
import { tencentOcrPages } from "./convert/ocr-tencent.js";
import { volcOcrPages } from "./convert/ocr-volc.js";
...
async function cacheLocationOf(cwd) {
  try {
    const { loadCacheLocation } = await import("./settings.js");
    return await loadCacheLocation(cwd);
  } catch {
    return "home";
  }
}
...
async function textResultToResponse(ctx, bytes, input, cwd, textResult, directLimit) {
  ...
  const { root, rel } = resolveCacheRoot(cwd, await cacheLocationOf(cwd));
      ...
      if (rendered.pages.length > 0) {
        ...
        if (useBaidu) {
          try {
            ...
            const jpegPages = loaded.ok
              ? await Promise.all(rendered.pages.map(async (page) => ({
                  data: await loaded.mod(Buffer.from(page.data)).jpeg({ quality: 85 }).toBuffer()
                })))
              : rendered.pages.map((page) => ({ data: Buffer.from(page.data) }));
        ...
        // 阿里云 / 腾讯 / Azure / 火山（auto 下按配置顺序尝试，显式指定时仅试对应项）
        if (ocr === null && policy.aliyun?.appCode && (policy.ocr === "aliyun" || policy.ocr === "auto")) {
          try {
            const loaded = await loadSharp();
            const jpegPages = loaded.ok
              ? await Promise.all(rendered.pages.map(async (page) => ({
            ...
            const sections = results.map((e, i) => ({ i, t: e.text.trim() })).filter(x => x.t !== "").map(x => `<!-- p${x.i + 1} -->\n${x.t}`);
            const chars = results.reduce((s, e) => s + e.text.length, 0);
            const confidence = results.length > 0 ? results.reduce((s, e) => s + e.confidence, 0) / results.length : 0;
            ocr = { text: sections.join("\n\n"), chars, confidence };
            ocrEngine = "aliyun";
            ocrNote = "阿里云 OCR（AppCode）";
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            ocrNote = `阿里云 OCR 不可用（${message}），已回退`;
            ctx.logger?.warn?.(`dsh-attachment-formats: aliyun OCR failed for ${input}: ${message}`);
          }
        }
        if (ocr === null && policy.tencent?.secretId && policy.tencent?.secretKey && (policy.ocr === "tencent" || policy.ocr === "auto")) {
          try {
            const loaded = await loadSharp();
            const jpegPages = loaded.ok
              ? await Promise.all(rendered.pages.map(async (page) => ({
              ...
              : rendered.pages.map((page) => ({ data: Buffer.from(page.data) }));
            const results = await tencentOcrPages(jpegPages, { secretId: policy.tencent.secretId, secretKey: policy.tencent.secretKey, region: policy.tencent.region });
            const sections = results.map((e, i) => ({ i, t: e.text.trim() })).filter(x => x.t !== "").map(x => `<!-- p${x.i + 1} -->\n${x.t}`);
            const chars = results.reduce((s, e) => s + e.text.length, 0);
            const confidence = results.length > 0 ? results.reduce((s, e) => s + e.confidence, 0) / results.length : 0;
            ocr = { text: sections.join("\n\n"), chars, confidence };
            ...
            const message = error instanceof Error ? error.message : String(error);
            ocrNote = `腾讯云 OCR 不可用（${message}），已回退`;
            ctx.logger?.warn?.(`dsh-attachment-formats: tencent OCR failed for ${input}: ${message}`);
          }
        }
        if (ocr === null && policy.azure?.apiKey && policy.azure?.endpoint && (policy.ocr === "azure" || policy.ocr === "auto")) {
          try {
            const loaded = await loadSharp();
            const jpegPages = loaded.ok
              ? await Promise.all(rendered.pages.map(async (page) => ({
            ...
            ocr = { text: sections.join("\n\n"), chars, confidence };
            ocrEngine = "azure";
            ocrNote = "Azure Document Intelligence";
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            ocrNote = `Azure OCR 不可用（${message}），已回退`;
            ctx.logger?.warn?.(`dsh-attachment-formats: azure OCR failed for ${input}: ${message}`);
          }
        }
        if (ocr === null && policy.volc?.appCode && (policy.ocr === "volc" || policy.ocr === "auto")) {
          try {
            const loaded = await loadSharp();
            const jpegPages = loaded.ok
              ? await Promise.all(rendered.pages.map(async (page) => ({
            ...
            const message = error instanceof Error ? error.message : String(error);
            ocrNote = `火山 OCR 不可用（${message}），已回退`;
            ctx.logger?.warn?.(`dsh-attachment-formats: volc OCR failed for ${input}: ${message}`);
          }
        }
        // DeepSeek Vision（零配置：复用宿主 DeepSeek Key，失败回退本地）
        // 显式 deepseek 恒试；auto 下探测到 Key 且未关闭「auto 启用视觉」才试，
        // 避免离线 smoke 与未配 Key 的部署被拦截。
        const dsExplicit = policy.ocr === "deepseek";
        const dsAutoOk = policy.deepseekAuto !== false;
        let dsKey = "";
        if (ocr === null && (dsExplicit || (policy.ocr === "auto" && dsAutoOk))) {
          dsKey = await resolveDeepSeekKey(policy.deepseek?.key || policy.vlm?.key || "", ctx);
        }
        if (ocr === null && (dsExplicit || (policy.ocr === "auto" && dsAutoOk && dsKey !== ""))) {
          try {
            const loaded = await loadSharp();
            const jpegPages = loaded.ok
              ? await Promise.all(rendered.pages.map(async (page) => ({
            ...
            ocrNote = "已用 DeepSeek Vision 转录（deepseek-v4-flash-vision-exp，表格已转 GFM，按 token 计费）";
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            // 显式模式失败要说明原因；auto 探测失败静默回退（不噪音）
            if (dsExplicit) ocrNote = `DeepSeek Vision 不可用（${message}），已回退`;
            ctx.logger?.warn?.(`dsh-attachment-formats: deepseek OCR failed for ${input}: ${message}`);
          }
        }
        if (ocr === null && (policy.ocr === "auto" || policy.ocr === "tesseract-js")) {
          const local = await ocrPages(rendered.pages, { onPage });
          ocr = local;
...
export async function executeAttachCommand(ctx, invocation) {
  ...
  if (verb === "" || verb === "list") {
    try {
      const docs = await listCachedDocs(cwd, await cacheLocationOf(cwd));
      if (docs.length === 0) {
  ...
  if (verb === "full") {
    ...
    try {
      ...
      const cached = await readCachedDoc(cwd, doc.id, await cacheLocationOf(cwd));
    ...
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          await ensureCacheMigrated(cwd, await cacheLocationOf(cwd));
  ...
  ctx.effect(
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        const loc = await cacheLocationOf(cwd);
        await ensureCacheMigrated(cwd, loc);
        const docs = await listCachedDocs(cwd, loc).catch(() => []);
        const { root } = resolveCacheRoot(cwd, await cacheLocationOf(cwd));
        sendJson(res, 200, {
          ok: true,
          docs,
    ...
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const removed = await removeCachedDocs(cwd, Array.isArray(body?.ids) ? body.ids : [], await cacheLocationOf(cwd));
    ...
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const cleared = await clearCache(cwd, await cacheLocationOf(cwd));
    ...
    () => ctx.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const cacheLocation = await cacheLocationOf(cwd);
      ...
      handler: async (req, res) => {
        ...
        if (req.method === "GET") {
          try {
            ...
            const { config, revision } = await loadSettingsDoc(cwd);
        ...
        if (req.method === "POST") {
          try {
            ...
            let next = await saveSettings(cwd, patch, expectedRevision);
            ...
            if (creds !== undefined && creds !== null && typeof creds.set === "function") {
              ...
              if (moved > 0) next = await saveSettings(cwd, clearPatch);
            }
            sendJson(res, 200, { ok: true, config: redactForClient(next), revision: next.revision });
          } catch (error) {
